...
<details>
<summary><strong>m_casting</strong> (업캐스팅 · 다운캐스팅 · instanceof)</summary>

## 기본 개념  
### ※ 모든 자식은 부모 타입이다.
- 자식 객체는 부모 타입으로 담을 수 있다.  
- 반대로, 부모 객체를 자식 타입으로는 절대 담을 수 없다.
...
### 1. Up Casting (업캐스팅)
- 자식 타입 객체를 부모 타입으로 형변환하는 것.  
- 자식에게 구현된 필드는 따로 존재하고, 부모가 가진 필드와 메소드만 접근 가능하다.

#### 특징
- 자동 형변환된다.  
- 여러 자식들을 하나의 부모 타입으로 묶고 싶을 때 사용한다.

---

### 2. Down Casting (다운캐스팅)
- 부모 타입(업캐스팅된 상태)에 담겨있는 자식 객체를 다시 자식 타입으로 되돌리는 형변환.  
- 업캐스팅 시 따로 분리되었던 자식 필드들이 다시 붙어서 자식 타입으로 복구된다.

#### 특징
- 반드시 업캐스팅된 객체만 다운캐스팅할 수 있다.  
- 부모 객체를 억지로 자식 타입으로 변환하면 오류 발생.

---

## Casting을 잘 사용하는 법
- 모든 자식 객체를 하나의 부모 타입으로 묶을 때 업캐스팅을 진행한다.  
- 전달받은 객체가 어떤 자식 타입인지 검사한 뒤 해당 타입으로 다운캐스팅을 진행한다.  
- 즉, **묶어서 받고, 필요할 때 다시 원래 타입으로 복구하는 방식**이다.

---

## 타입 비교 연산자 instanceof: 조건식
- 객체명 instanceof 클래스명
...
<details>
<summary><strong>n_bank_practice</strong> (종합 실습 문제)</summary>
...
<details>
<summary><strong>o_abstract_class</strong> (추상 클래스)</summary>
...
<details>
<summary><strong>l_interface</strong> (인터페이스)</summary>
...
- 이 때 중간에서 강제성을 없애주는 클래스를 **추상 클래스**로 선언하기로 하며,  
  추상 클래스 이름 뒤에는 **Adapter**를 붙여서 목적을 알려준다.
...
<details>
<summary><strong>p_interface</strong> (인터페이스)</summary>
